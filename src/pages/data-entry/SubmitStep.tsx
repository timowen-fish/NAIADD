...
import { saveSurveySession } from "../../services/surveySessionService";
import type {
  SubmissionValidationIssue,
  SurveySubmission,
} from "../../types/submission";
import type { SurveySession } from "../../types/surveySession";
import "../../styles/SubmitStep.css";
...
type Stage = "package" | "validate" | "queue";
...
export default function SubmitStep({
  ...
  const rows = useMemo(
    () =>
      Array.isArray(session.specimens)
  ...
  async function submitToDba(): Promise<void> {
    setMessage("");
    setWarnings([]);
    ...
    if (!currentUser) {
      setMessage(
        "Your sign-in session is no longer available. Sign in again before submitting.",
      );
      setScreenState("failed");
      return;
    }
  ...
  if (screenState === "submitting") {
    ...
    return (
      <main className="submitStep submitCenteredState">
        <section className="submitProgressCard">
          ...
          <div className="submitStageList">
            <StageRow
              complete={stage !== "package"}
              active={stage === "package"}
              text="Building submission package"
            />
            <StageRow
              complete={stage === "queue"}
              active={stage === "validate"}
              text="Validating survey data"
            />
            <StageRow
              complete={false}
              active={stage === "queue"}
              text="Adding survey to Firestore queue"
            />
          </div>
        </section>
      </main>
      ...
      <main className="submitStep submitCenteredState">
        <section className="submitResultCard success">
          ...
          <div className="submitResultSummary">
            ...
            <ResultRow
              label="Queued"
              value={formatDateTime(queuedAt)}
            />
          </div>

          {warnings.length > 0 && (
            <div className="submitInlineMessage success">
              <span>!</span>
    ...
    return (
      <main className="submitStep submitCenteredState">
        <section className="submitResultCard danger">
          ...
          {message && (
            <div className="submitInlineMessage danger">
              {message}
            </div>
          )}
  ...
  return (
    <main className="submitStep">
      <button
        type="button"
        className="submitBackButton"
        onClick={onBack}
      >
        ← Back to Review
      </button>
      ...
      <div className="submitChoiceGrid">
        <section className="submitChoiceCard draft">
          ...
          <button
            type="button"
            className="submitSecondaryAction"
...
function SummaryItem({
  ...
  return (
    <div className="submitSummaryItem">
      <span>{label}</span>
