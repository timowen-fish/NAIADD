...
import {
  analyzeCollections,
  loadCollectionEfforts,
  type AnalysisGrouping,
  type AnalysisProgress,
  type AnalysisResult,
  type CollectionEffortValue,
  type EffortMethod,
} from "../services/analysisEngine";
import "../styles/CPUEPage.css";

const GROUPING_OPTIONS: Array<{ value: AnalysisGrouping; label: string }> = [
...
type SecondaryGrouping = AnalysisGrouping | "none";
type VisualMetric = "cpue" | "cpueS" | "cpueQ" | "cpueP" | "cpueM" | "cpueT";
type ChartType = "line" | "bar";
type ChartDimension = "primary" | "secondary";

const VISUAL_METRICS: Array<{ value: VisualMetric; label: string }> = [
...
function groupingLabel(grouping: AnalysisGrouping): string {
  return GROUPING_OPTIONS.find((option) => option.value === grouping)?.label ?? grouping;
}

function compareGroupValues(left: string, right: string): number {
...
function detectedForMethod(item: CollectionEffortValue, method: EffortMethod): number | null {
  if (method === "hour") return item.detectedHour;
  if (method === "net_night") return item.detectedNetNight;
  return item.detectedDistanceMeters;
}

function chooseDefaultMethod(values: CollectionEffortValue[]): EffortMethod {
  const counts: Record<EffortMethod, number> = { hour: 0, net_night: 0, kilometer: 0 };
  values.forEach((item) => {
    if (item.detectedHour != null) counts.hour += 1;
    if (item.detectedNetNight != null) counts.net_night += 1;
    if (item.detectedDistanceMeters != null) counts.kilometer += 1;
  });
  return (Object.entries(counts) as Array<[EffortMethod, number]>).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "hour";
}

export default function CPUEPage() {
  const [appliedQuery, setAppliedQuery] = useState<AppliedQueryData | null>(() => loadAppliedQueryData());
  const [grouping, setGrouping] = useState<AnalysisGrouping>("year");
  const [secondaryGrouping, setSecondaryGrouping] =
    useState<SecondaryGrouping>("species");
  const [effortMethod, setEffortMethod] = useState<EffortMethod>("hour");
  const [effortValues, setEffortValues] = useState<CollectionEffortValue[]>([]);
  const [effortOverrides, setEffortOverrides] = useState<Record<string, number>>({});
  ...
  const [visualMetric, setVisualMetric] = useState<VisualMetric>("cpue");
  ...
  useEffect(() => {
    const refresh = () => setAppliedQuery(loadAppliedQueryData());
    window.addEventListener(APPLIED_QUERY_DATA_EVENT, refresh);
    window.addEventListener("storage", refresh);
    return () => {
      window.removeEventListener(APPLIED_QUERY_DATA_EVENT, refresh);
  ...
  useEffect(() => {
    ...
    loadCollectionEfforts(collectionIDs, controller.signal, setProgress)
      .then((values) => {
        ...
        values.forEach((item) => {
          ...
          if (value != null) defaults[item.collectionID] = value;
        });
        setEffortOverrides(defaults);
      })
      .catch((error) => {
        if (!isAbortError(error)) setErrorMessage(error instanceof Error ? error.message : String(error));
      })
      .finally(() => {
        if (!controller.signal.aborted) setLoadingEffort(false);
      });
  }, [appliedQuery?.appliedAt]);

  function changeEffortMethod(next: EffortMethod) {
  ...
  function updateEffort(collectionID: string, rawValue: string) {
    setResult(null);
    setEffortOverrides((current) => {
      const next = { ...current };
      if (rawValue.trim() === "") delete next[collectionID];
      else {
        const value = Number(rawValue);
        if (Number.isFinite(value)) next[collectionID] = value;
      }
      return next;
    });
  }
  ...
  const summary = useMemo(() => {
    if (!result) return { collections: 0, fish: 0, effort: 0, groups: 0, cpue: 0 };
  ...
  function exportCsv() {
    ...
    anchor.href = url;
    anchor.download = `VADMA_CPUE_${grouping}${
      secondaryGrouping === "none" ? "" : `_${secondaryGrouping}`
  ...
  const chartModel = useMemo(() => {
    ...
    const hasSecondary = result.metrics.some(
    ...
    const xValue = (metric: AnalysisResult["metrics"][number]) =>
      effectiveXDimension === "primary"
        ? metric.primaryGroup
        : metric.secondaryGroup ?? "All";
    const seriesValue = (metric: AnalysisResult["metrics"][number]) =>
      !hasSecondary
        ? "CPUE"
    ...
    const series = seriesNames.map((name) => ({
      ...
      values: categories.map(
        (category) =>
          valueLookup.get(`${category}\u001f${name}`) ?? null,
      ),
    }));
  ...
  async function calculateMetrics() {
    if (!hasAppliedQuery || loadingEffort || isCalculating || missingEffortCount > 0) return;
    const controller = new AbortController();
    abortControllerRef.current = controller;
    ...
    try {
      const calculated = await analyzeCollections({
        collectionIDs,
        grouping,
        secondaryGrouping:
      ...
      if (isAbortError(error)) setErrorMessage("Analysis cancelled.");
      else setErrorMessage(error instanceof Error ? error.message : String(error));
    } finally {
  ...
  return (
    <section className="cpue-page" aria-labelledby="cpue-page-title">
      ...
      <section className="cpue-settings-card">
        ...
        <div className="cpue-control-grid">
          <label className="cpue-field">
            ...
            <select
              ...
              onChange={(event) => {
                const next = event.target.value as AnalysisGrouping;
                setGrouping(next);
          ...
          <label className="cpue-field">
            ...
            <select
              ...
              disabled={isCalculating || grouping === "overall"}
              onChange={(event) => {
                setSecondaryGrouping(
                  event.target.value as SecondaryGrouping,
                );
                setResult(null);
              }}
            >
        ...
        {effortValues.length > 0 && (
          <div className="cpue-effort-editor">
        ...
        {progress && (
          <div className="cpue-progress-panel" aria-live="polite">
      ...
      {showVisualization && result && (
        <div
          className="cpue-visualization-backdrop"
          role="presentation"
          onMouseDown={(event) => {
        ...
        >
          <section
            className="cpue-visualization-modal"
            role="dialog"
            aria-modal="true"
            aria-labelledby="cpue-visualization-title"
          >
            <div className="cpue-visualization-header">
              ...
              <button
                type="button"
                className="cpue-visualization-close"
            ...
            <div className="cpue-visualization-controls">
              <label>
                ...
                <select
                  ...
                  onChange={(event) =>
                    setVisualMetric(
                      event.target.value as VisualMetric,
                    )
                  }
                >
              ...
              <label>
                ...
                <select
                  value={xDimension}
                  disabled={secondaryGrouping === "none"}
                  onChange={(event) =>
                    setXDimension(
                      event.target.value as ChartDimension,
                    )
                  }
                >
            ...
            <div className="cpue-chart-shell">
              ...
              chartModel.series.length === 0 ? (
                <div className="cpue-histogram-empty">
                  No values are available for this configuration.
                </div>
              ) : (
                <svg
                  className="cpue-chart"
                  viewBox={`0 0 ${chartGeometry.width} ${chartGeometry.height}`}
                  role="img"
                  aria-label={`${VISUAL_METRICS.find((item) => item.value === visualMetric)?.label ?? "CPUE"} chart`}
                >
                  {[0, 0.25, 0.5, 0.75, 1].map((fraction) => {
                    const value =
                      chartModel.maximum * (1 - fraction);
                    const y =
                  ...
                  {chartType === "line"
                    ? chartModel.series.map(
                        (series, seriesIndex) => {
                          ...
                          return (
                            <g key={series.name}>
                              <path
                                className={`cpue-chart-line ${chartSeriesClass(seriesIndex)}`}
                                d={path}
                              />
                              {points.map((point, pointIndex) => (
                                <circle
                                  key={pointIndex}
                                  className={`cpue-chart-point ${chartSeriesClass(seriesIndex)}`}
                                  cx={point.x}
                                  cy={point.y}
                                  r={5}
                                >
                    ...
                    : chartModel.categories.flatMap(
                        (category, categoryIndex) => {
                          const usableWidth =
                            chartGeometry.categoryStep * 0.78;
                          const barWidth = Math.max(
                            3,
                            usableWidth /
                              Math.max(
                                1,
                                chartModel.series.length,
                              ),
                          );
                            ...
                            (series, seriesIndex) => {
                              ...
                              return (
                                <rect
                                  ...
                                  className={`cpue-chart-bar ${chartSeriesClass(seriesIndex)}`}
                                  x={x}
                                  y={y}
