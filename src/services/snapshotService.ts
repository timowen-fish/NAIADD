...
export type SnapshotSyncStatus = {
...
type SnapshotCacheRecord = {
  id: typeof SNAPSHOT_RECORD_ID;
  meta: SnapshotMetadata;
  blob: Blob;
};

type CollectionIndexCacheRecord = {
  id: typeof COLLECTION_INDEX_RECORD_ID;
  version: string;
  cachedAt: string;
  records: CollectionIndexRecord[];
};

type CacheRecord = SnapshotCacheRecord | CollectionIndexCacheRecord;
...
async function readCacheRecord<T extends CacheRecord>(
  id: string,
): Promise<T | null> {
...
async function writeCacheRecord(record: CacheRecord): Promise<void> {
  const database = await openSnapshotDatabase();
...
async function deleteCacheRecord(id: string): Promise<void> {
  const database = await openSnapshotDatabase();
...
function validateConfiguration(value: unknown): SnapshotConfiguration {
  ...
  if (
    typeof configuration.snapshotUrl !== "string" ||
  ...
  if (
    typeof configuration.snapshotKey !== "string" ||
    !/^[0-9a-fA-F]{128}$/.test(configuration.snapshotKey.trim())
  ) {
    throw new Error(
      "Snapshot configuration requires a 128-character hexadecimal CBC/HMAC key.",
    );
  }
  ...
  return {
    ...
    snapshotIndexUrl:
      typeof configuration.snapshotIndexUrl === "string" &&
...
function parseEncryptedPayload(text: string): EncryptedPayload {
...
async function decryptPayload(
  ...
  const hmacKey = await crypto.subtle.importKey(
    "raw",
    bytesToArrayBuffer(hmacKeyBytes),
    {
      name: "HMAC",
      hash: "SHA-256",
    },
...
function normalizeIndexRecords(value: unknown): CollectionIndexRecord[] {
  ...
  return value
    ...
    .map((item) => {
      ...
      } as CollectionIndexRecord;
    });
}

function coerceParquetValue(value: unknown): unknown {
  return typeof value === "bigint" ? Number(value) : value;
}

function coerceParquetRow(row: DataRow): DataRow {
...
async function getAvailableParquetColumns(
  file: ArrayBuffer,
): Promise<string[] | null> {
  try {
    const hyparquet: typeof import("hyparquet") = await import("hyparquet");
...
export async function fetchSnapshotConfiguration(): Promise<SnapshotConfiguration> {
  const reference = doc(db, "snapshotKeys", "current");
...
export async function downloadCollectionIndex(
  snapshotIndexUrl: string,
  snapshotKey: string,
): Promise<CollectionIndexRecord[]> {
...
export function getCachedSnapshotMetadata(): SnapshotMetadata | null {
  ...
  try {
    return JSON.parse(stored) as SnapshotMetadata;
  } catch {
...
export async function getCachedCollectionIndex(): Promise<
  CollectionIndexRecord[] | null
> {
  const record = await readCacheRecord<CollectionIndexCacheRecord>(
    COLLECTION_INDEX_RECORD_ID,
  );
...
async function cacheCollectionIndex(
  version: string,
  records: CollectionIndexRecord[],
): Promise<void> {
  await writeCacheRecord({
...
export async function ensureCollectionIndex(): Promise<
  CollectionIndexRecord[]
> {
  const configuration = await fetchSnapshotConfiguration();
  const cached =
    await readCacheRecord<CollectionIndexCacheRecord>(
      COLLECTION_INDEX_RECORD_ID,
    );

  if (
    cached?.version === configuration.version &&
  ...
  if (!configuration.snapshotIndexUrl) {
    throw new Error(
      "snapshotIndexUrl is missing from Firestore snapshotKeys/current.",
    );
  }
...
export async function syncSnapshotIfNeeded(): Promise<SnapshotSyncStatus> {
  ...
  if (!navigator.onLine) {
    return cachedMeta
      ...
      : {
          state: "offline",
          message:
            "No production snapshot is cached. Connect once to download it.",
        };
  }
  ...
  if (
    cachedMeta?.version === configuration.version &&
...
export async function forceSyncSnapshot(
  suppliedConfiguration?: SnapshotConfiguration,
  ...
  return {
    state: "updated",
    message: `Downloaded and decrypted snapshot ${meta.version} (${formatMegabytes(
      meta.sizeBytes,
    )} MB).`,
...
export async function readSnapshotRows(
  ...
  const hyparquet: typeof import("hyparquet") = await import("hyparquet");
  const compressorModule: typeof import("hyparquet-compressors") =
...
export async function readSnapshotCollectionRows(
  collectionID: string,
): Promise<DataRow[]> {
  ...
  const match = index.find(
    (record) => record.CollectionID === normalizedCollectionID,
  );

  if (!match) {
    throw new Error(
      `CollectionID ${normalizedCollectionID} was not found in the collection index.`,
    );
  }
...
export async function readSnapshotColumnNames(): Promise<string[]> {
  const cached = await getCachedSnapshot();
