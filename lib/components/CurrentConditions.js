...
require("../styles/CurrentConditions.css");
...
function fuzzySiteScore(site, query) {
...
function windArrowSymbol(direction) {
...
function cssFlowDegreesFromWindFrom(directionDegrees) {
    return (directionDegrees + 90 + 360) % 360;
}
function interpolateWindAt(vectors, x, y, fallbackDirectionDegrees, fallbackSpeedMph) {
    if (!vectors.length) {
...
function WindOverlay({ enabled, speedMph, directionDegrees, vectors, }) {
    ...
    (0, react_1.useEffect)(() => {
        ...
        const resetParticle = (particle, seed, soft = false) => {
        ...
        const colorForSpeed = (speed, alpha = 1) => {
            if (speed < 2)
                return `rgba(190, 225, 255, ${0.12 * alpha})`;
            if (speed < 7)
                return `rgba(68, 230, 130, ${0.48 * alpha})`;
            if (speed < 14)
                return `rgba(246, 222, 73, ${0.62 * alpha})`;
            if (speed < 24)
                return `rgba(255, 139, 40, ${0.72 * alpha})`;
        ...
        const draw = (now) => {
            ...
            particlesRef.current.forEach((particle, index) => {
                ...
                const localWind = interpolateWindAt(vectors, percentX, percentY, directionDegrees, speedMph);
                const direction = ((localWind.cssDegrees +
                    Math.sin(particle.phase * 6.283 + now * 0.00035) * 5) *
                    Math.PI) /
                    180;
                const speed = Math.max(0.5, localWind.speedMph || speedMph || 8);
                const velocity = (compact ? 0.68 : 0.78) *
                ...
                const alpha = Math.min(0.86, Math.max(0.18, 0.18 + speed / 32)) *
...
function coopsStationToTideStation(item) {
    const id = String(item?.id ?? item?.stationId ?? item?.station_id ?? "").trim();
...
function readCachedTideStations() {
    try {
...
function writeCachedTideStations(stations) {
    try {
        localStorage.setItem(TIDE_STATION_CACHE_KEY, JSON.stringify({
...
async function loadNoaaTideStations() {
...
function tideStationScore(station, lat, lng, label) {
...
function parseUsgsRdbSites(text, originLat, originLng) {
    const lines = text
        .split(/\r?\n/)
        .filter((line) => line && !line.startsWith("#"));
    if (lines.length < 3)
        return [];
    const headerIndex = lines.findIndex((line) => line.includes("site_no") && line.includes("dec_lat_va"));
...
function formatFlowNumber(value, suffix) {
    if (!Number.isFinite(Number(value)))
        return "Not reported";
    const rounded = Math.abs(Number(value)) >= 100
...
function getUsgsVariableCode(item) {
...
function parseUsgsTimeSeriesValues(item) {
    const valueBlocks = Array.isArray(item?.values) ? item.values : [];
    const rawValues = valueBlocks.flatMap((block) => Array.isArray(block?.value) ? block.value : []);
...
async function fetchUsgsIvSeries(siteId, days = 7) {
    const end = new Date();
...
async function fetchNearestUsgsFlowGage(lat, lng) {
    ...
    for (const radius of searchRadii) {
        ...
        url.searchParams.set("bBox", `${(lng - radius).toFixed(4)},${(lat - radius).toFixed(4)},${(lng + radius).toFixed(4)},${(lat + radius).toFixed(4)}`);
        ...
        for (const candidate of sites) {
            try {
                ...
                if ((candidate.flowSeries?.length ?? 0) >= 2 ||
                    (candidate.stageSeries?.length ?? 0) >= 2) {
                    return candidate;
                }
            }
            catch (error) {
                console.warn(`Unable to load USGS hydrograph for ${candidate.id}:`, error);
            }
        }
        if (sites.length > 0) {
            return sites[0];
...
function TideChart({ predictions }) {
    if (predictions.length < 2) {
        return (<div className="home-tide-empty">
    ...
    const points = predictions.map((item, index) => {
        const x = paddingLeft + (index / Math.max(predictions.length - 1, 1)) * plotWidth;
        const y = paddingTop + ((max - item.value) / scaledSpread) * plotHeight;
        return { ...item, x, y };
    });
    const controlPoint = (current, previous, next, reverse = false) => {
        const smoothing = 0.16;
        const opposedLineLength = Math.hypot(next.x - previous.x, next.y - previous.y);
        const opposedLineAngle = Math.atan2(next.y - previous.y, next.x - previous.x);
    ...
    const extrema = points
        ...
        if (point.value >= previous.value && point.value >= next.value)
            return { ...point, kind: "High" };
        if (point.value <= previous.value && point.value <= next.value)
            return { ...point, kind: "Low" };
        return null;
    })
        .filter(Boolean)
        .slice(0, 4);
    ...
    const gridValues = Array.from({ length: 5 }, (_, index) => min + (scaledSpread * index) / 4).reverse();
    const formatTime = (time) => new Date(time).toLocaleTimeString([], {
        hour: "numeric",
        minute: "2-digit",
    });
    const formatTickTime = (time) => new Date(time).toLocaleTimeString([], { hour: "numeric" });
    return (<svg className="home-tide-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="24 hour tide prediction chart">
      <defs>
        <linearGradient id="vadmaTideArea" x1="0" x2="0" y1="0" y2="1">
          <stop offset="0%" stopColor="var(--vadma-cc-water)" stopOpacity="0.28"/>
          <stop offset="100%" stopColor="var(--vadma-cc-water)" stopOpacity="0.02"/>
        </linearGradient>
      ...
      <rect x={paddingLeft} y={paddingTop} width={plotWidth} height={plotHeight} rx="10" className="home-tide-plot-bg"/>

      {gridValues.map((value) => {
            const y = paddingTop + ((max - value) / scaledSpread) * plotHeight;
            return (<g key={`tide-grid-${value.toFixed(3)}`}>
            <line x1={paddingLeft} x2={width - paddingRight} y1={y} y2={y} className="home-tide-grid-line"/>
            <text x={paddingLeft - 12} y={y + 4} textAnchor="end" className="home-tide-axis-label">
              {value.toFixed(1)} ft
            </text>
          </g>);
        })}

      <line x1={paddingLeft} x2={width - paddingRight} y1={baselineY} y2={baselineY} className="home-tide-axis"/>
      ...
      {extrema.map((point, index) => {
            ...
            const labelX = Math.min(Math.max(point.x, paddingLeft + 46), width - paddingRight - 46);
            const anchor = labelX < point.x - 2
                ? "end"
                : labelX > point.x + 2
                    ? "start"
                    : "middle";
            return (<g key={`${point.kind}-${point.time}-${index}`} className={isHigh ? "home-tide-peak high" : "home-tide-peak low"}>
            <line x1={point.x} x2={point.x} y1={point.y} y2={labelY - 5} className="home-tide-peak-leader"/>
            <circle cx={point.x} cy={point.y} r="3.7"/>
            <text x={labelX} y={labelY} textAnchor={anchor} className="home-tide-peak-label">
              {point.kind} {formatTime(point.time)}
            </text>
            <text x={labelX} y={labelY + 11} textAnchor={anchor} className="home-tide-peak-value">
              {point.value.toFixed(1)} ft
            </text>
          </g>);
        })}

      {tickPoints.map((point, index) => (<text key={`${point.time}-tick-${index}`} x={point.x} y={height - 15} textAnchor={index === 0
                ? "start"
                : index === tickPoints.length - 1
                    ? "end"
                    : "middle"} className="home-tide-tick">
          {formatTickTime(point.time)}
        </text>))}
    </svg>);
}
function FlowHydrograph({ points, unit, label, }) {
    ...
    const minimumRange = unit === "ft" ? 0.12 : Math.max(1, Math.abs(scaleMax) * 0.015);
    ...
    const clampForScale = (value) => Math.min(paddedMax, Math.max(paddedMin, value));
    const xFor = (index) => paddingLeft + (index / Math.max(points.length - 1, 1)) * chartWidth;
    const yFor = (value) => paddingTop +
    ...
    const valueTicks = [0, 0.25, 0.5, 0.75, 1].map((fraction) => paddedMin + fraction * paddedRange);
    ...
    const gradientId = unit === "cfs" ? "vadmaFlowDischargeLine" : "vadmaFlowStageLine";
    const areaId = unit === "cfs" ? "vadmaFlowDischargeArea" : "vadmaFlowStageArea";
    return (<div className="home-flow-hydrograph-wrap">
      ...
      <svg className="home-flow-hydrograph" viewBox={`0 0 ${width} ${height}`} preserveAspectRatio="none" role="img" aria-label={`${label} hydrograph`}>
        <defs>
          ...
          <linearGradient id={areaId} x1="0" x2="0" y1="0" y2="1">
            <stop offset="0%" stopColor="var(--vadma-cc-water-soft)" stopOpacity="0.28"/>
            <stop offset="100%" stopColor="var(--vadma-cc-water)" stopOpacity="0.02"/>
          </linearGradient>
        </defs>

        <rect className="home-flow-plot-bg" x={paddingLeft} y={chartTop} width={chartRight - paddingLeft} height={chartBottom - chartTop} rx="12"/>

        {valueTicks.map((value, index) => {
            const y = yFor(value);
            return (<g key={`${label}-value-${index}`}>
              <line className="home-flow-grid-line" x1={paddingLeft} x2={chartRight} y1={y} y2={y}/>
              <text className="home-flow-value-label" x={paddingLeft - 12} y={y + 4} textAnchor="end">
        ...
        {tickIndexes.map((index) => {
            ...
            return (<g key={`${point.time}-${label}-tick`}>
              <line className="home-flow-time-grid" x1={point.x} x2={point.x} y1={chartTop} y2={chartBottom}/>
              <text x={point.x} y={height - 15} textAnchor={index === 0
                    ? "start"
                    : index === points.length - 1
                        ? "end"
                        : "middle"} className="home-flow-time-tick">
        ...
        <line className="home-flow-axis-line" x1={paddingLeft} x2={chartRight} y1={chartBottom} y2={chartBottom}/>
        <line className="home-flow-axis-line" x1={paddingLeft} x2={paddingLeft} y1={chartTop} y2={chartBottom}/>
        <path className="home-flow-area" d={areaPath} fill={`url(#${areaId})`}/>
        <path className="home-flow-line" d={linePath} stroke={`url(#${gradientId})`}/>
        <circle className="home-flow-latest-dot" cx={latest.x} cy={latest.y} r="5"/>
...
function formatCompactFlowValue(value, unit) {
...
function FlowGagePanel({ loading, gage, error, }) {
    return (<div className="home-flow-gage-panel">
      ...
      {gage ? (<div className="home-flow-gage-card">
          ...
          <div className="home-flow-hydrographs">
            {gage.flowSeries && gage.flowSeries.length >= 2 ? (<FlowHydrograph points={gage.flowSeries} unit="cfs" label="Discharge hydrograph"/>) : null}
            {gage.stageSeries && gage.stageSeries.length >= 2 ? (<FlowHydrograph points={gage.stageSeries} unit="ft" label="Gage height hydrograph"/>) : null}
            {(!gage.flowSeries || gage.flowSeries.length < 2) &&
                (!gage.stageSeries || gage.stageSeries.length < 2) ? (<div className="home-flow-hydrograph-empty">
                No recent hydrograph values were returned for this gage.
              </div>) : null}
          </div>

          <a className="home-flow-gage-link" href={gage.url} target="_blank" rel="noreferrer">
            Open USGS gage
          </a>
        </div>) : (<div className="home-flow-gage-empty">
          {loading
...
function HourlyForecastChart({ points }) {
    if (points.length < 2) {
        return null;
    }
    ...
    const plotted = points.map((point, index) => {
        ...
        const tempY = paddingTop + ((maxTemp - point.temperature) / tempSpread) * tempHeight;
        const windY = windTop +
    ...
    const tempPath = plotted
        .map((point, index) => `${index === 0 ? "M" : "L"} ${point.x.toFixed(1)} ${point.tempY.toFixed(1)}`)
        .join(" ");
    const windPath = plotted
        .map((point, index) => `${index === 0 ? "M" : "L"} ${point.x.toFixed(1)} ${point.windY.toFixed(1)}`)
        .join(" ");
    const tickEvery = Math.max(1, Math.ceil(points.length / 6));
    return (<div className="home-hourly-forecast-wrap">
      <div className="home-hourly-forecast-meta">
        <strong>48-hour weather graph</strong>
        <span>{points.length} hourly points loaded</span>
      </div>
      <svg className="home-hourly-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="48-hour temperature, wind speed, and wind direction forecast graph">
        <defs>
        ...
        <text x={paddingLeft} y={windTop - 16} className="home-hourly-section-label">
          Wind speed / direction
        </text>

        {tempTicks.map((tick) => {
            const y = paddingTop + ((maxTemp - tick) / tempSpread) * tempHeight;
            return (<g key={`temp-${tick}`}>
              <line x1={paddingLeft} x2={width - paddingRight} y1={y} y2={y} className="home-hourly-grid-line"/>
              <text x={paddingLeft - 10} y={y + 4} textAnchor="end" className="home-hourly-axis-label">
                {tick}°
              </text>
            </g>);
        })}
        ...
        {windTicks.map((tick) => {
            const y = windTop + ((maxWind - tick) / Math.max(maxWind, 1)) * windHeight;
            return (<g key={`wind-${tick}`}>
              <line x1={paddingLeft} x2={width - paddingRight} y1={y} y2={y} className="home-hourly-grid-line"/>
              <text x={paddingLeft - 10} y={y + 4} textAnchor="end" className="home-hourly-axis-label">
                {tick} mph
              </text>
        ...
        {plotted.map((point, index) => index % tickEvery === 0 ? (<g key={`${point.time}-${index}`}>
              <circle cx={point.x} cy={point.tempY} r="3.5" className="home-hourly-temp-dot"/>
              <circle cx={point.x} cy={point.windY} r="3.5" className="home-hourly-wind-dot"/>
              <text x={point.x} y={height - 12} textAnchor="middle" className="home-hourly-tick">
                {new Date(point.time).toLocaleTimeString([], {
                hour: "numeric",
            })}
              </text>
              <text x={point.x} y={windTop + windHeight + 20} textAnchor="middle" className="home-hourly-wind-direction">
...
function createVadmaMapMarker(L, lat, lng) {
...
function CurrentConditions({ sites, }) {
    const mapRef = (0, react_1.useRef)(null);
    const mapInstanceRef = (0, react_1.useRef)(null);
    const radarLayerRef = (0, react_1.useRef)(null);
    const basemapLayerRef = (0, react_1.useRef)(null);
    const markerRef = (0, react_1.useRef)(null);
    const windRequestControllerRef = (0, react_1.useRef)(null);
    const windRequestIdRef = (0, react_1.useRef)(0);
    const windRefreshTimerRef = (0, react_1.useRef)(null);
    const [mapStatus, setMapStatus] = (0, react_1.useState)("Radar hidden. Toggle radar to view precipitation.");
    const [siteSearch, setSiteSearch] = (0, react_1.useState)("");
    const [selectedSiteKey, setSelectedSiteKey] = (0, react_1.useState)("");
    const [forecastTitle, setForecastTitle] = (0, react_1.useState)("Forecast");
    const [forecastText, setForecastText] = (0, react_1.useState)("Click the map or choose a sampling site to pull today's nearest National Weather Service forecast.");
    const [forecastLoading, setForecastLoading] = (0, react_1.useState)(false);
    const [showWind, setShowWind] = (0, react_1.useState)(true);
    const [mapReadyToken, setMapReadyToken] = (0, react_1.useState)(0);
    const [basemapMode, setBasemapMode] = (0, react_1.useState)("dark");
    const [showRadar, setShowRadar] = (0, react_1.useState)(false);
    const [windSpeedMph, setWindSpeedMph] = (0, react_1.useState)(null);
    const [windDirectionDegrees, setWindDirectionDegrees] = (0, react_1.useState)(225);
    const [windVectors, setWindVectors] = (0, react_1.useState)([]);
    const [lastPoint, setLastPoint] = (0, react_1.useState)(null);
    const [tideLoading, setTideLoading] = (0, react_1.useState)(false);
    const [tideTitle, setTideTitle] = (0, react_1.useState)("Nearest tide station");
    const [tideText, setTideText] = (0, react_1.useState)("Choose a sampling site or click the map to refresh the nearest 24-hour tide chart.");
    const [tidePredictions, setTidePredictions] = (0, react_1.useState)([]);
    const [tideStations, setTideStations] = (0, react_1.useState)(TIDE_STATIONS);
    const [flowLoading, setFlowLoading] = (0, react_1.useState)(false);
    const [flowGage, setFlowGage] = (0, react_1.useState)(null);
    const [flowError, setFlowError] = (0, react_1.useState)("Choose a sampling site or click the map to search for the nearest USGS flow gage.");
    const basemapLabel = basemapMode === "dark"
        ? "Carto dark"
        : basemapMode === "topo"
            ? "Topo"
            : "Satellite";
    const nextBasemapMode = () => {
        setBasemapMode((current) => current === "dark" ? "topo" : current === "topo" ? "satellite" : "dark");
    };
    const [hourlyForecast, setHourlyForecast] = (0, react_1.useState)([]);
    (0, react_1.useEffect)(() => {
        let cancelled = false;
    ...
    const getBasemapLayer = (0, react_1.useCallback)((L) => {
        if (basemapMode === "topo") {
    ...
    const requestWindField = (0, react_1.useCallback)(async () => {
        ...
        try {
            ...
            url.searchParams.set("longitude", samples.map((sample) => sample.lng.toFixed(4)).join(","));
    ...
    const scheduleWindRefresh = (0, react_1.useCallback)((delay = 200) => {
        if (!showWind)
            return;
    ...
    const matchingSites = (0, react_1.useMemo)(() => {
    ...
    const requestTides = (0, react_1.useCallback)(async (lat, lng, label = "Selected location") => {
        const now = new Date();
        ...
        try {
            ...
            [...stationsForSearch, ...TIDE_STATIONS].forEach((station) => mergedStations.set(station.id, station));
            ...
            for (const candidate of sortedStations) {
                ...
                url.searchParams.set("format", "json");
                const tryPredictionRequest = async (interval, useDatum) => {
                    ...
                    */
                    ...
                    try {
                        ...
                        if (!response.ok) {
                            console.warn("NOAA tide request failed", candidate.station.id, candidate.station.name, interval, useDatum ? "with datum" : "without datum", response.status);
                            return [];
                        }
                        const data = await response.json();
                        if (data?.error?.message) {
                            console.warn("NOAA tide station returned no prediction data", candidate.station.id, candidate.station.name, data.error.message);
                            return [];
                        }
                        ...
                        console.warn("NOAA tide request threw", candidate.station.id, candidate.station.name, error);
                        return [];
                    }
                };
            ...
            if (!selected || predictions.length < 2) {
                throw new Error("No tide predictions were returned for the nearest NOAA stations.");
            }
            setTidePredictions(predictions);
            setTideTitle(selected.station.name);
            setTideText(`Nearest NOAA tide station with predictions, ${selected.distanceMiles.toFixed(1)} miles from ${label}.`);
        }
        catch (error) {
            console.warn("Unable to fetch tide predictions:", error);
            setTideTitle("Tides unavailable");
            setTideText(error instanceof Error
                ? error.message
    ...
    const requestFlowGage = (0, react_1.useCallback)(async (lat, lng, label = "Selected location") => {
        ...
        try {
            ...
            if (!gage) {
                setFlowGage(null);
                setFlowError("No active USGS streamflow or stage gage was found nearby.");
                return;
            }
            setFlowGage(gage);
            setFlowError("");
    ...
    const requestForecast = (0, react_1.useCallback)(async (lat, lng, label = "Selected location") => {
        ...
        try {
            ...
            const pointResponse = await fetch(`https://api.weather.gov/points/${normalizedLat.toFixed(4)},${normalizedLng.toFixed(4)}`, {
                headers: {
                    Accept: "application/geo+json, application/json",
                },
            });
            if (!pointResponse.ok) {
                throw new Error("The National Weather Service could not resolve that point.");
            }
            ...
            setWindDirectionDegrees(windDirectionToDegrees(firstPeriod.windDirection));
            ...
            if (forecastHourlyUrl) {
                ...
                if (hourlyResponse.ok) {
                    ...
                    setHourlyForecast(hourlyPeriods
                        .slice(0, 48)
                        .map((period) => ({
                        ...
                        .filter((period) => period.time && Number.isFinite(period.temperature)));
                }
            }
            setForecastTitle(place);
            setForecastText(`${firstPeriod.name}: ${firstPeriod.shortForecast}. ${firstPeriod.temperature}°${firstPeriod.temperatureUnit}. Wind ${firstPeriod.windSpeed} ${firstPeriod.windDirection}.${tonight}`);
            ...
            setForecastTitle("Forecast unavailable");
            setHourlyForecast([]);
            setForecastText(error instanceof Error
                ? error.message
    ...
    (0, react_1.useEffect)(() => {
        ...
        const ensureLeaflet = async () => {
            ...
            await new Promise((resolve, reject) => {
                const existing = document.querySelector('script[data-vadma-leaflet="true"]');
                if (existing) {
        ...
        const buildMap = async () => {
            try {
                ...
                radarLayerRef.current = L.tileLayer(`https://tilecache.rainviewer.com${latestFrame.path}/256/{z}/{x}/{y}/2/1_1.png`, {
                    opacity: 0.58,
                    zIndex: 10,
                    maxNativeZoom: 8,
                    maxZoom: 10,
                ...
                mapInstanceRef.current.on("zoomend", () => {
                    ...
                    const hasRadar = mapInstanceRef.current.hasLayer(radarLayerRef.current);
    ...
    (0, react_1.useEffect)(() => {
        const map = mapInstanceRef.current;
        if (!showWind || !map || mapReadyToken === 0)
            return undefined;
        ...
        const idleRefreshTimer = window.setInterval(() => scheduleWindRefresh(0), 5 * 60 * 1000);
        return () => {
    ...
    function selectSite(site) {
        ...
        if (L && map) {
            ...
            markerRef.current = createVadmaMapMarker(L, site.lat, site.lng).addTo(map);
        }
        void requestForecast(site.lat, site.lng, site.label);
    }
    return (<div className="home-current-conditions">
      <div className="home-current-toolbar">
        <div className="home-site-search-wrap">
          <label className="home-site-search-label" htmlFor="home-site-forecast-search">
            Forecast by sampling site
          </label>
          <input id="home-site-forecast-search" className="home-site-forecast-search" value={siteSearch} onChange={(event) => {
            setSiteSearch(event.target.value);
            setSelectedSiteKey("");
        }} placeholder="Type Boshers, Manchester, City Dock, SiteID, waterbody..." aria-label="Search sampling sites for forecast"/>

          <div className="home-site-search-results">
            {matchingSites.length > 0 ? (matchingSites.map((site) => (<button type="button" className={site.key === selectedSiteKey
                ? "home-site-search-result active"
                : "home-site-search-result"} key={site.key} onClick={() => selectSite(site)}>
                ...
                </button>))) : (<div className="home-site-search-empty">
        ...
        <div className="home-condition-buttons">
          <button type="button" className={showWind ? "home-wind-toggle active" : "home-wind-toggle"} onClick={() => setShowWind((current) => {
            const next = !current;
            if (!next) {
                windRequestControllerRef.current?.abort();
                windRequestControllerRef.current = null;
          ...
          <button type="button" className={showRadar ? "home-wind-toggle active" : "home-wind-toggle"} onClick={() => setShowRadar((current) => !current)} aria-pressed={showRadar}>
            {showRadar ? "Hide radar" : "Show radar"}
            <span>RainViewer precipitation</span>
          </button>

          <button type="button" className="home-wind-toggle active" onClick={nextBasemapMode} aria-label="Change baselayer">
      ...
      <div className={`home-current-map-wrap ${basemapMode}-basemap`}>
        ...
        <WindOverlay enabled={showWind} speedMph={windSpeedMph} directionDegrees={windDirectionDegrees} vectors={windVectors}/>
      ...
      <div className="home-tide-panel">
        <button type="button" className="home-tide-toggle active" onClick={() => {
            if (lastPoint) {
...
//# sourceMappingURL=CurrentConditions.js.map