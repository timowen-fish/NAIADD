...
exports.WORKFLOW_SESSION_EVENT = exports.WORKFLOW_STEP_EVENT = void 0;
exports.getSurveyDraftMetadata = getSurveyDraftMetadata;
exports.createSurveySession = createSurveySession;
exports.listSurveyDrafts = listSurveyDrafts;
exports.listSurveyDraftRecords = listSurveyDraftRecords;
exports.loadSurveySession = loadSurveySession;
exports.saveSurveySession = saveSurveySession;
exports.createSurveyDraft = createSurveyDraft;
exports.activateSurveyDraft = activateSurveyDraft;
exports.duplicateSurveyDraft = duplicateSurveyDraft;
exports.deleteSurveyDraft = deleteSurveyDraft;
exports.getSurveyDraft = getSurveyDraft;
exports.getSurveyDraftRecord = getSurveyDraftRecord;
exports.clearSurveySession = clearSurveySession;
exports.requestWorkflowStep = requestWorkflowStep;
const ACTIVE_SESSION_PREFIX = "naiadd.surveySession";
const DRAFTS_PREFIX = "naiadd.surveyDrafts";
exports.WORKFLOW_STEP_EVENT = "naiadd-workflow-step";
exports.WORKFLOW_SESSION_EVENT = "naiadd-workflow-session";
function activeSessionKey(uid) {
...
function createCollectionId(uid) {
    return `COLL_${submitterCode(uid)}_${timestampCode(new Date())}_${randomCode()}`;
}
...
function isSurveySession(value, ownerUid) {
...
function normalizeSession(session) {
    ...
    return {
        ...session,
        collectionId: session.collectionId || createCollectionId(session.ownerUid),
...
function readDrafts(uid) {
    try {
        ...
        return parsed
            .filter((item) => isSurveySession(item, uid))
...
function writeDrafts(uid, sessions) {
    ...
    const sorted = Array.from(unique.values()).sort((left, right) => right.updatedAt.localeCompare(left.updatedAt));
    localStorage.setItem(draftsKey(uid), JSON.stringify(sorted));
    return sorted;
}
function upsertDraft(session) {
    const drafts = readDrafts(session.ownerUid);
    const existingIndex = drafts.findIndex((draft) => draft.id === session.id);
    if (existingIndex >= 0) {
...
function writeActiveSession(session) {
    localStorage.setItem(activeSessionKey(session.ownerUid), JSON.stringify(session));
}
function dispatchSessionEvent(session) {
    window.dispatchEvent(new CustomEvent(exports.WORKFLOW_SESSION_EVENT, {
        detail: session,
    }));
}
function countFish(session) {
    return session.specimens.reduce((total, specimen) => {
        const row = asRecord(specimen);
        const commonName = textValue(row, ["CommonName", "commonName"], "");
        if (!commonName || commonName === "NoFish") {
            return total;
        }
        return (total +
            (numericValue(firstValue(row, ["Quantity", "quantity"])) ?? 1));
    }, 0);
}
function countSpecies(session) {
...
function getSurveyDraftMetadata(session) {
    const normalized = normalizeSession(session);
    const location = asRecord(normalized.location);
    const survey = asRecord(normalized.survey);
    const waterbody = textValue(location, ["Waterbody", "waterbody"], "Unnamed Waterbody");
    const siteName = textValue(location, ["SiteName", "siteName"], "Unnamed Site");
    const surveyDate = textValue(survey, [
        "Survey_Date",
        "SurveyDate",
        "surveyDate",
        "Date",
    ...
    const searchableText = [
        ...
        ...normalized.specimens.map((specimen) => textValue(asRecord(specimen), ["CommonName", "commonName"], "")),
    ]
        .join(" ")
        .toLowerCase();
    return {
...
function createSurveySession(uid) {
    const now = new Date().toISOString();
    return {
 ...
 */
function listSurveyDraftRecords(uid) {
 ...
 */
function loadSurveySession(uid) {
    try {
 ...
 */
function saveSurveySession(session) {
 ...
 */
function createSurveyDraft(uid) {
 ...
 */
function activateSurveyDraft(uid, sessionId) {
    const session = readDrafts(uid).find((draft) => draft.id === sessionId);
    if (!session) {
 ...
 */
function duplicateSurveyDraft(uid, sessionId) {
    const source = readDrafts(uid).find((draft) => draft.id === sessionId);
    if (!source) {
        throw new Error("Survey draft not found.");
    }
    const now = new Date().toISOString();
    const duplicate = normalizeSession({
        ...source,
        id: crypto.randomUUID(),
        collectionId: createCollectionId(uid),
        ownerUid: uid,
        currentStep: source.currentStep === "submit"
            ? "review"
            : source.currentStep,
        createdAt: now,
        updatedAt: now,
 ...
 */
function deleteSurveyDraft(uid, sessionId) {
    const remaining = readDrafts(uid).filter((draft) => draft.id !== sessionId);
    writeDrafts(uid, remaining);
    try {
        const rawActive = localStorage.getItem(activeSessionKey(uid));
        if (rawActive) {
            const active = JSON.parse(rawActive);
            if (isSurveySession(active, uid) &&
                active.id === sessionId) {
                localStorage.removeItem(activeSessionKey(uid));
            }
        }
 ...
 */
function getSurveyDraft(uid, sessionId) {
    return (readDrafts(uid).find((draft) => draft.id === sessionId) ??
        null);
}
/**
 * Returns the derived metadata for one saved draft.
 */
function getSurveyDraftRecord(uid, sessionId) {
 ...
 */
function clearSurveySession(uid) {
    localStorage.removeItem(activeSessionKey(uid));
    dispatchSessionEvent();
}
function requestWorkflowStep(step) {
    window.dispatchEvent(new CustomEvent(exports.WORKFLOW_STEP_EVENT, {
        detail: step,
    }));
}
//# sourceMappingURL=surveySessionService.js.map