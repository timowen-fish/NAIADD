...
exports.default = SpecimenStandardStep;
const react_1 = require("react");
const FishEntryModal_1 = __importDefault(require("./FishEntryModal"));
require("../../styles/SpecimenStandardStep.css");
...
function SpecimenStandardStep({ siteID, onBack, onContinueToSaveDraft, draftFishRows = [], }) {
    ...
    return (<main className="app specimenStandardStep">
          ...
          {runs.map((run, runIndex) => (<section key={runIndex} className="runGroup">
              <div className="runGroupHeader">
                ...
                <label className="runNBox">
                  CustomRunName
                  <input value={run.customRunName} onChange={(e) => {
                const value = e.target.value;
                setRuns((prev) => prev.map((existingRun, index) => index === runIndex
      ...
      {modalOpen && (<FishEntryModal_1.default activeRun={activeRun} activePass={activePass} rows={modalRows} onRowsChange={setModalRows} onSave={saveModal} onClose={() => setModalOpen(false)}/>)}
    </main>);
}
//# sourceMappingURL=SpecimenStandardStep.js.map