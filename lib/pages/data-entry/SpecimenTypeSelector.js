...
exports.default = SpecimenTypeSelector;
require("../../styles/SpecimenTypeSelector.css");
const specimenTypes = [
    {
        id: "standard",
        icon: "🐟",
...
function SpecimenTypeSelector({ selectedType, onSelect, onContinue, onBack, }) {
    return (<main className="specimen-type-selector">
      ...
      <section className="specimen-type-card-shell">
        <div className="specimen-type-grid" role="radiogroup" aria-label="Specimen entry method">
          {specimenTypes.map((option) => {
            ...
            return (<button key={option.id} type="button" role="radio" aria-checked={selected} className={`specimen-type-card${selected ? " selected" : ""}`} onClick={() => onSelect(option.id)}>
                ...
                <span className="specimen-type-copy">
                  ...
                  <span className="specimen-type-detail-list">
                    {option.details.map((detail) => (<span key={detail}>• {detail}</span>))}
                  </span>
                </span>
              </button>);
        })}
        ...
        <div className="specimen-type-actions">
          <div>
            {selectedType ? (<span>A specimen entry method is selected.</span>) : (<span>Select one method to continue.</span>)}
          </div>

          <button type="button" className="specimen-type-continue" disabled={!selectedType} onClick={onContinue}>
            Continue →
          </button>
        </div>
      </section>
...
//# sourceMappingURL=SpecimenTypeSelector.js.map