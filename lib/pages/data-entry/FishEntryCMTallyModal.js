...
require("../../styles/FishEntryCMTallyModal.css");
...
function buildGridFromRows(rows) {
    const species = Array.from(new Set(rows
        .map((row) => row.CommonName?.trim())
        .filter((value) => Boolean(value))));
    const cmBins = uniqueSortedNumbers(rows
    ...
    rows.forEach((row) => {
        ...
        if (!speciesName || !Number.isFinite(cm))
            return;
        if (!grid[speciesName]) {
            grid[speciesName] = {};
        }
...
function getSpeciesQuantityTotal(speciesName, grid) {
    return Object.values(grid[speciesName] || {}).reduce((sum, value) => sum + Number(value || 0), 0);
}
function rowsFromGrid(species, cmBins, grid, scientificLookup) {
    const rows = [];
...
function FishEntryCMTallyModal({ activeRun, activePass, rows, onRowsChange, onSave, onClose, }) {
    const [activeVadmaTheme, setActiveVadmaTheme] = (0, react_1.useState)(getActiveVadmaThemeName);
    const initial = (0, react_1.useMemo)(() => buildGridFromRows(rows), [rows]);
    const [speciesRecords, setSpeciesRecords] = (0, react_1.useState)([]);
    const [species, setSpecies] = (0, react_1.useState)(initial.species);
    const [cmBins, setCmBins] = (0, react_1.useState)(initial.cmBins);
    const [grid, setGrid] = (0, react_1.useState)(initial.grid);
    const [newSpecies, setNewSpecies] = (0, react_1.useState)("");
    const [isSpeciesPickListOpen, setIsSpeciesPickListOpen] = (0, react_1.useState)(false);
    const [highlightedSpeciesIndex, setHighlightedSpeciesIndex] = (0, react_1.useState)(0);
    const [newCmBin, setNewCmBin] = (0, react_1.useState)("");
    const [sortMode, setSortMode] = (0, react_1.useState)(null);
    (0, react_1.useEffect)(() => {
    ...
    const speciesNames = (0, react_1.useMemo)(() => uniqueClean(speciesRecords.map((x) => x.CommonName)), [speciesRecords]);
    const newSpeciesMatches = (0, react_1.useMemo)(() => {
    ...
    function selectNewSpecies(name) {
        setNewSpecies(name);
    ...
    const displayedSpecies = (0, react_1.useMemo)(() => {
        ...
        if (sortMode === "quantity") {
            return copied.sort((a, b) => {
                const quantityDifference = getSpeciesQuantityTotal(b, grid) - getSpeciesQuantityTotal(a, grid);
    ...
    function syncRows(nextSpecies, nextCmBins, nextGrid) {
        onRowsChange(rowsFromGrid(nextSpecies, nextCmBins, nextGrid, scientificLookup));
    }
    function addSpecies() {
    ...
    function updateQuantity(speciesName, cm, value) {
    ...
    function adjustQuantity(speciesName, cm, delta) {
    ...
    return (<div className="cmTallyOverlay" data-vadma-theme={activeVadmaTheme}>
      <section className="cmTallyModal">
        ...
        <section className="cmTallyTools">
          <label className="cmTallyTool">
            ...
            <div className="cmTallyInputRow">
              <div className="cmTallySpeciesPickList">
                <input value={newSpecies} placeholder="Type common name..." autoComplete="off" onFocus={() => setIsSpeciesPickListOpen(true)} onChange={(event) => {
            ...
            if (isSpeciesPickListOpen &&
                newSpeciesMatches.length > 0 &&
                event.key === "ArrowDown") {
                event.preventDefault();
                setHighlightedSpeciesIndex((current) => Math.min(current + 1, newSpeciesMatches.length - 1));
                return;
            }
            if (isSpeciesPickListOpen &&
                newSpeciesMatches.length > 0 &&
                event.key === "ArrowUp") {
                event.preventDefault();
                setHighlightedSpeciesIndex((current) => Math.max(current - 1, 0));
                return;
            }
            if (isSpeciesPickListOpen &&
                newSpeciesMatches.length > 0 &&
                event.key === "Enter") {
                event.preventDefault();
                selectNewSpecies(newSpeciesMatches[highlightedSpeciesIndex] ||
                    newSpeciesMatches[0]);
                return;
            }
            if (event.key === "Enter") {
                ...
                {isSpeciesPickListOpen && newSpecies.trim().length >= 2 && (<div className="cmTallySpeciesPickListDropdown">
                    {newSpeciesMatches.length > 0 ? (<>
                        <div className="cmTallySpeciesPickListHeader">
                          Matching cached species
                        </div>

                        {newSpeciesMatches.map((name, index) => (<button key={name} type="button" className={index === highlightedSpeciesIndex
                        ? "cmTallySpeciesPickListOption active"
                        : "cmTallySpeciesPickListOption"} onMouseDown={(event) => {
                        event.preventDefault();
          ...
          <label className="cmTallyTool">
            ...
            <div className="cmTallyInputRow">
              <input value={newCmBin} type="number" inputMode="numeric" placeholder="Example: 13" onChange={(event) => setNewCmBin(event.target.value)} onKeyDown={(event) => {
            if (event.key === "Enter") {
                event.preventDefault();
                addCmBin();
            }
        ...
        <section className="cmTallyTableShell">
          <table className="cmTallyTable">
            <colgroup>
              ...
              {displayedSpecies.map((speciesName) => (<col key={speciesName} className="cmTallySpeciesCol"/>))}
            </colgroup>
            ...
            <thead>
              <tr>
                ...
                {displayedSpecies.map((speciesName) => (<th key={speciesName}>
                    <div className="speciesHeaderCell">
                      <span title={speciesName}>{speciesName}</span>

                      <button type="button" title={`Remove ${speciesName}`} onClick={() => removeSpecies(speciesName)}>
                        ×
                      </button>
                    </div>
                  </th>))}
            ...
            <tbody>
              {cmBins.map((cm) => (<tr key={cm}>
                  <th className="cmBinCell">
                    <div className="cmBinCellInner">
                      <span>{cm} cm</span>

                      <button type="button" title={`Remove ${cm} cm bin`} onClick={() => removeCmBin(cm)}>
                        ×
                      </button>
                    </div>
                  </th>

                  {displayedSpecies.map((speciesName) => (<td key={`${speciesName}-${cm}`}>
                      <div className="cmTallyQuantityBox">
                        <button type="button" className="cmTallyMinus" onClick={() => adjustQuantity(speciesName, cm, -1)} aria-label={`Decrease ${speciesName} ${cm} cm tally`}>
                          −
                        </button>

                        <input type="number" inputMode="numeric" min="0" value={grid[speciesName]?.[cm] || ""} onChange={(event) => updateQuantity(speciesName, cm, event.target.value)}/>

                        <button type="button" className="cmTallyPlus" onClick={() => adjustQuantity(speciesName, cm, 1)} aria-label={`Increase ${speciesName} ${cm} cm tally`}>
                          +
                        </button>
                      </div>
                    </td>))}
          ...
          {species.length === 0 || cmBins.length === 0 ? (<div className="cmTallyEmpty">
              Add at least one species and one centimeter bin to begin.
            </div>) : null}
        </section>

        <footer className="cmTallyActions">
          <div className="cmTallySortControls" aria-label="Sort species columns">
            <span>Sort By:</span>

            <button type="button" className={sortMode === "az" ? "active" : ""} onClick={() => setSortMode(sortMode === "az" ? null : "az")}>
              A-Z
            </button>

            <button type="button" className={sortMode === "quantity" ? "active" : ""} onClick={() => setSortMode(sortMode === "quantity" ? null : "quantity")}>
              Quantity
            </button>
          </div>

          <div className="cmTallyActionButtons">
            <button type="button" className="cmTallySecondary" onClick={onClose}>
              Cancel
            </button>
...
exports.default = FishEntryCMTallyModal;
//# sourceMappingURL=FishEntryCMTallyModal.js.map