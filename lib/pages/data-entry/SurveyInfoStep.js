...
require("../../styles/SurveyInfoStep.css");
...
function readRequiredSurveyCache() {
    try {
...
function writeRequiredSurveyCache(values) {
    try {
...
function MultiPicklist({ title, required = false, options, selected, onChange, note, }) {
    const [open, setOpen] = (0, react_1.useState)(false);
    const [search, setSearch] = (0, react_1.useState)("");
    const safeSelected = (0, react_1.useMemo)(() => {
    ...
    const safeOptions = Array.isArray(options)
        ? options
            ...
            .filter(Boolean)
        : [];
    const filteredOptions = safeOptions.filter((option) => option.toLowerCase().includes(search.toLowerCase()));
    ...
    return (<div className="multiSelectField">
      <label className="multiSelectLabel">
        {title}
        {required ? " *" : ""}
      </label>

      <button type="button" className="multiSelectButton" onClick={() => setOpen((current) => !current)}>
        <span>
      ...
      {safeSelected.length > 0 && (<div className="multiSelectSelected">
          {safeSelected.map((item) => (<button key={item} type="button" className="multiSelectChip" onClick={() => toggleValue(item)}>
              {item} ×
            </button>))}
        </div>)}

      {open && (<div className="multiSelectMenu">
          <input className="multiSelectSearch" value={search} onChange={(e) => setSearch(e.target.value)} placeholder={`Search ${title.toLowerCase()}...`}/>

          <div className="multiSelectOptions">
            {filteredOptions.length === 0 ? (<div className="multiSelectEmpty">No matches found.</div>) : (filteredOptions.map((option) => (<button key={option} type="button" className={`multiSelectOption ${safeSelected.includes(option) ? "active" : ""}`} onClick={() => toggleValue(option)}>
...
function SurveyInfoStep({ location, onBack, initialSurvey, onSurveySaved, }) {
    const today = new Date().toISOString().slice(0, 10);
    const [lists, setLists] = (0, react_1.useState)(fallbackLists);
    const [saveMessage, setSaveMessage] = (0, react_1.useState)("");
    const [validationMessage, setValidationMessage] = (0, react_1.useState)("");
    const [listMessage, setListMessage] = (0, react_1.useState)("Loading reference-data lists...");
    const cachedRequiredFields = (0, react_1.useMemo)(() => readRequiredSurveyCache(), []);
    ...
    (0, react_1.useEffect)(() => {
        ...
        async function loadLists() {
            ...
            try {
                const referenceData = await (0, referenceDataService_1.loadReferenceDataResilient)();
                if (cancelled)
                    return;
                const general = referenceData.generalLists;
                const rawLists = {
                    Surveyors: (0, referenceDataService_1.getReferenceList)(general, [
                        "surveyors",
                        "surveyor",
                        "lead biologist",
                        "lead biologists",
                    ...
                    Project: (0, referenceDataService_1.getReferenceList)(general, ["project", "projects"]),
                    SamplingMethod: (0, referenceDataService_1.getReferenceList)(general, [
                        "sampling method",
                        "sampling methods",
                        "samplingmethod",
                        "gear type",
                    ...
                    SurveyType: (0, referenceDataService_1.getReferenceList)(general, [
                        "survey type",
                        "survey types",
                        "surveytype",
                    ]),
                    Equipment: (0, referenceDataService_1.getReferenceList)(general, [
                        "equipment",
                        "equipment list",
                    ]),
                    TargetSpecies: (0, referenceDataService_1.getReferenceList)(general, [
                        "target species",
                        "targetspecies",
                        "target species list",
                    ]),
                    Weather: (0, referenceDataService_1.getReferenceList)(general, [
                        "weather",
                        "condition",
                        "conditions",
                    ]),
                    Wind: (0, referenceDataService_1.getReferenceList)(general, ["wind"]),
                    HabitatStructure: (0, referenceDataService_1.getReferenceList)(general, [
                        "habitat structure",
                        "habitatstructure",
                    ]),
                    BankStability: (0, referenceDataService_1.getReferenceList)(general, [
                        "bank stability",
                        "bankstability",
                    ]),
                    LandUse: (0, referenceDataService_1.getReferenceList)(general, ["land use", "landuse"]),
                    CanopyCover: (0, referenceDataService_1.getReferenceList)(general, [
                        "canopy cover",
                        "canopycover",
                    ]),
                };
                ...
                console.info("VADMA Survey Reference Data", {
                    availableKeys,
                    connectedGroups,
                    counts: Object.fromEntries(Object.entries(general).map(([key, values]) => [
                        key,
                        values.length,
                    ])),
                });
                setListMessage(connectedGroups.length > 0
                ...
                if (!cancelled) {
                    ...
                    setListMessage(error instanceof Error
    ...
    (0, react_1.useEffect)(() => {
        setForm((current) => ({
            ...
            TargetSpeciesNew: current.TargetSpeciesNew || lists.TargetSpecies[0] || "None",
            Wind: current.Wind || lists.Wind[0] || "NA",
            HabitatStructure: current.HabitatStructure || lists.HabitatStructure[0] || "NA",
    ...
    const totalPersonHours = (0, react_1.useMemo)(() => {
    ...
    function updateMulti(field, value) {
        setForm((current) => {
    ...
    const numberInput = (field, label) => (<label>
      {label}
      <input className="input" type="number" value={String(form[field] ?? "")} onChange={(e) => updateField(field, e.target.value)}/>
    </label>);
    const codeInput = (field, label) => (<label>
      {label}
      <input className="input" type="number" min="0" max="6" step="1" value={String(form[field] ?? "")} onChange={(e) => updateField(field, e.target.value)}/>
    </label>);
    function validateAndSave() {
        ...
        if (!form.Lead_Collector)
            missing.push("Lead Biologist");
        if (surveyors.length === 0)
            missing.push("Surveyors");
        if (!form.Project)
            missing.push("Project");
        if (!form.Survey_Type)
            missing.push("Survey Type");
        if (gearTypes.length === 0)
            missing.push("Sampling Method");
        if (!form.TargetSpeciesNew)
            missing.push("Target Species");
        if (missing.length > 0) {
            setSaveMessage("");
        ...
        const surveyInfo = {
            ...
            SubmergedTerrestrialVegetation: form.SubmergedTerrestrialVegetation || null,
        ...
        localStorage.setItem("vadma2.currentSurveyInfo", JSON.stringify({
    ...
    return (<main className="app surveyInfoStep">
      {onBack && (<button type="button" className="backButton" onClick={onBack}>
          ← Back to Location
        </button>)}

      {location && (<section className="existingSiteCard savedLocationCard">
      ...
      <section className="existingSiteCard requiredFieldsCard">
        ...
        <div className="cacheHelperBar">
          ...
          <button type="button" className="cacheHelperButton" onClick={applyCachedRequiredFields}>
        ...
        <div className="detailsGrid requiredDetailsGrid">
          <label>
            Survey Date *
            <input className="input" type="date" value={form.Survey_Date} onChange={(e) => updateField("Survey_Date", e.target.value)}/>
          </label>

          <label>
            Lead Biologist *
            <select className="input" value={form.Lead_Collector} onChange={(e) => updateField("Lead_Collector", e.target.value)}>
              <option value="">Select Lead Biologist</option>

              {lists.Surveyors.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Project *
            <select className="input" value={form.Project} onChange={(e) => updateField("Project", e.target.value)}>
              <option value="">Select Project</option>

              {lists.Project.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Survey Type *
            <select className="input" value={form.Survey_Type} onChange={(e) => updateField("Survey_Type", e.target.value)}>
              <option value="">Select Survey Type</option>

              {lists.SurveyType.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Target Species *
            <select className="input" value={form.TargetSpeciesNew} onChange={(e) => updateField("TargetSpeciesNew", e.target.value)}>
              <option value="">Select Target Species</option>

              {lists.TargetSpecies.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
        ...
        <MultiPicklist title="Surveyors" required options={lists.Surveyors} selected={form.Surveyors} onChange={(nextSelected) => setForm((current) => ({
            ...current,
            Surveyors: nextSelected,
        }))} note="Select all crew members involved in the survey."/>

        <MultiPicklist title="Sampling Method" required options={lists.SamplingMethod} selected={form.Geartype} onChange={(nextSelected) => setForm((current) => ({
            ...current,
      ...
      <details className="existingSiteCard">
        ...
        <div className="detailsGrid detailsPad">
          <label>
            Survey Start Time
            <input className="input" type="time" value={form.Survey_StartTime} onChange={(e) => updateField("Survey_StartTime", e.target.value)}/>
          </label>

          <label>
            Survey End Time
            <input className="input" type="time" value={form.Survey_EndTime} onChange={(e) => updateField("Survey_EndTime", e.target.value)}/>
          </label>

          <label>
            Effort Seconds
            <input className="input" type="number" value={form.Effort} onChange={(e) => updateField("Effort", e.target.value)}/>
          </label>

          <label>
            Number of Surveyors
            <input className="input" type="number" value={form.Survey_NumSurveyors} onChange={(e) => updateField("Survey_NumSurveyors", e.target.value)}/>
          </label>
          ...
          <label>
            ...
            <input className="input" type="number" min="1" step="1" value={form.TotalPasses} onChange={(e) => updateField("TotalPasses", e.target.value)}/>
          </label>

          <label>
            Comments
            <input className="input" value={form.Comments} onChange={(e) => updateField("Comments", e.target.value)}/>
          </label>
        </div>
      </details>
      ...
      <details className="existingSiteCard">
        ...
        <div className="detailsGrid detailsPad">
          <label>
            Equipment
            <select className="input" value={form.Equip} onChange={(e) => updateField("Equip", e.target.value)}>
              <option value="">Select Equipment</option>

              {lists.Equipment.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            ACDC
            <select className="input" value={form.ACDC} onChange={(e) => updateField("ACDC", e.target.value)}>
              <option>DC</option>
              <option>AC</option>
            </select>
          </label>
          ...
          <label>
            Volts
            <input className="input" type="number" value={form.Volts} onChange={(e) => updateField("Volts", e.target.value)}/>
          </label>

          <label>
            Amps
            <input className="input" type="number" value={form.Amps} onChange={(e) => updateField("Amps", e.target.value)}/>
          </label>
          ...
          <label>
            PPS
            <input className="input" type="number" value={form.PPS} onChange={(e) => updateField("PPS", e.target.value)}/>
          </label>

          <label>
            Range
            <input className="input" type="number" value={form.Range} onChange={(e) => updateField("Range", e.target.value)}/>
          </label>
        </div>
      </details>
      ...
      <details className="existingSiteCard">
        ...
        <div className="detailsGrid detailsPad">
          ...
          <label>
            Wind
            <select className="input" value={form.Wind} onChange={(e) => updateField("Wind", e.target.value)}>
              {lists.Wind.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Tide
            <select className="input" value={form.Tide} onChange={(e) => updateField("Tide", e.target.value)}>
        ...
        <div className="checkboxGroup">
          ...
          {lists.Weather.map((x) => (<label key={x} className="checkPill">
              <input type="checkbox" checked={form.Weather.includes(x)} onChange={() => updateMulti("Weather", x)}/>
              {x}
            </label>))}
        </div>
        ...
        <details className="existingSiteCard nestedDetailsCard">
          ...
          <div className="detailsGrid detailsPad">
            ...
            {numberInput("SpecificConductivity_uScm", "Specific Conductivity µS/cm")}
      ...
      <details className="existingSiteCard">
        ...
        <div className="detailsGrid detailsPad">
          <label>
            Habitat Structure
            <select className="input" value={form.HabitatStructure} onChange={(e) => updateField("HabitatStructure", e.target.value)}>
              {lists.HabitatStructure.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Bank Stability
            <select className="input" value={form.BankStability} onChange={(e) => updateField("BankStability", e.target.value)}>
              {lists.BankStability.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Land Use
            <select className="input" value={form.LandUse} onChange={(e) => updateField("LandUse", e.target.value)}>
              {lists.LandUse.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
          ...
          <label>
            Canopy Cover
            <select className="input" value={form.CanopyCover} onChange={(e) => updateField("CanopyCover", e.target.value)}>
              {lists.CanopyCover.map((x) => (<option key={x} value={x}>
                  {x}
                </option>))}
            </select>
          </label>
        ...
        <details className="existingSiteCard nestedDetailsCard">
          ...
          <div className="detailsGrid detailsPad">
            ...
            {codeInput("SubmergedTerrestrialVegetation", "Submerged Terrestrial Vegetation")}
      ...
      <div className={`mobileStepFooter mobileStepFooterSingle ${validationMessage || saveMessage ? "mobileStepFooterWithMessage" : ""}`}>
        {(validationMessage || saveMessage) && (<div className={`mobileStepFooterMessage ${validationMessage
                ? "mobileStepFooterMessageError"
                : "mobileStepFooterMessageOk"}`} role={validationMessage ? "alert" : "status"}>
...
exports.default = SurveyInfoStep;
//# sourceMappingURL=SurveyInfoStep.js.map