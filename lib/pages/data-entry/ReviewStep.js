...
function ReviewStep({ session, onEditLocation, onEditSurvey, onEditSpecimens, onContinue, }) {
    const location = asRecord(session.location);
    const survey = asRecord(session.survey);
    const rows = (0, react_1.useMemo)(() => (Array.isArray(session.specimens) ? session.specimens.map(asRecord) : []), [session.specimens]);
    ...
    const totalFish = (0, react_1.useMemo)(() => realRows.reduce((sum, row) => sum + (numeric(firstValue(row, ["Quantity", "quantity"])) ?? 1), 0), [realRows]);
    const measuredRows = (0, react_1.useMemo)(() => realRows
        .map((row) => ({
        row,
        length: numeric(firstValue(row, ["Length", "length", "ForkLength"])),
    }))
        .filter((item) => item.length !== null), [realRows]);
    const largest = (0, react_1.useMemo)(() => measuredRows.length > 0
        ? measuredRows.reduce((best, item) => (item.length > best.length ? item : best))
        : null, [measuredRows]);
    const smallest = (0, react_1.useMemo)(() => measuredRows.length > 0
        ? measuredRows.reduce((best, item) => (item.length < best.length ? item : best))
        : null, [measuredRows]);
    const netCount = (0, react_1.useMemo)(() => new Set(realRows
        .map((row) => display(firstValue(row, ["NetNumber", "CustomRunName"]), ""))
        .filter(Boolean)).size, [realRows]);
    const panelCount = (0, react_1.useMemo)(() => new Set(realRows
        .map((row) => {
        ...
        .filter(Boolean)).size, [realRows]);
    const cmClassCount = (0, react_1.useMemo)(() => new Set(realRows
        .map((row) => numeric(firstValue(row, ["Length", "length"])))
        .filter((value) => value !== null)).size, [realRows]);
    const missingWeight = realRows.filter((row) => numeric(firstValue(row, ["Weight", "weight"])) === null).length;
    const missingSex = realRows.filter((row) => display(firstValue(row, ["Sex", "sex"]), "") === "").length;
    const latitude = numeric(firstValue(location, [
        "DownstreamLat",
        "downstreamLat",
        "DownstreamLatitude",
        "downstreamLatitude",
    ...
    const longitude = numeric(firstValue(location, [
        "DownstreamLong",
        "downstreamLong",
        "DownstreamLongitude",
        "downstreamLongitude",
    ...
    const lengthUnit = display(firstValue(realRows[0] ?? {}, ["LengthUnit", "lengthUnit"]), session.specimenFormType === "cm_tally" ? "cm" : "");
    return (<main className="reviewStep">
      ...
      <div className="reviewCardGrid">
        <ReviewCard title="Location" icon="⌖" onEdit={onEditLocation}>
          ...
          <div className="reviewMapPreview" aria-label="Location coordinate preview">
            {latitude !== null && longitude !== null ? (<react_leaflet_1.MapContainer center={[latitude, longitude]} zoom={15} minZoom={6} maxZoom={18} zoomControl={false} attributionControl={false} dragging={false} scrollWheelZoom={false} doubleClickZoom={false} touchZoom={false} keyboard={false} style={{
                width: "100%",
                height: "100%",
                minHeight: "220px",
                borderRadius: "18px",
                ...
                <react_leaflet_1.TileLayer url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}"/>

                <react_leaflet_1.CircleMarker center={[latitude, longitude]} radius={7} pathOptions={{
                color: "#ffffff",
                fillColor: "#ff9f43",
                fillOpacity: 1,
                opacity: 1,
              ...
              </react_leaflet_1.MapContainer>) : (<strong>Coordinates unavailable</strong>)}
          </div>
        </ReviewCard>

        <ReviewCard title="Survey Details" icon="▤" onEdit={onEditSurvey}>
          <Definition label="Survey Date" value={formatDate(firstValue(survey, ["Survey_Date", "SurveyDate", "surveyDate", "Date"]))}/>
        ...
        <ReviewCard title="Specimen Summary" icon="◇" onEdit={onEditSpecimens} wide>
          ...
          <div className="reviewSpeciesSummary">
            ...
            {speciesSummary.length === 0 ? (<p>No specimen rows have been entered.</p>) : (<div className="reviewSpeciesRows">
                {speciesSummary.slice(0, 12).map((species) => (<div key={species.name}>
        ...
        <section className="reviewValidationCard">
          ...
          <div className="reviewValidationList">
            {validationItems.map((item, index) => (<button key={`${item.text}-${index}`} type="button" className={item.level} onClick={item.action}>
...
function ReviewCard({ title, icon, onEdit, wide = false, children, }) {
    return (<section className={wide ? "reviewCard wide" : "reviewCard"}>
      <div className="reviewCardHeader">
        <div className="reviewCardTitle">
          <span>{icon}</span>
          <h2>{title}</h2>
...
function Definition({ label, value, mono = false, }) {
    return (<div className="reviewDefinition">
      <span>{label}</span>
      <strong className={mono ? "mono" : ""}>{display(value)}</strong>
    </div>);
}
function Metric({ label, value }) {
    return (<div className="reviewMetric">
      <span>{label}</span>
      <strong>{value}</strong>
    </div>);
}
